...
app = FastAPI(
    openapi_url='/api/v1/check-face-real-time/openapi.json',
    docs_url='/api/v1/check-face-real-time/docs',
    redoc_url='/api/v1/check-face-real-time/redoc',
...
@app.websocket('/api/v1/check-face-real-time/ws')
...
@app.websocket('/api/v1/check-face-real-time/test_ws')
...
# @app.get('/api/v1/check-face-real-time/camera')
...
@app.get('/api/v1/check-face-real-time/camera')
def video_feed(id_region: str,turn: str,rtsp: str):
    url = rtsp
    if url not in stream.keys() or stream[url].thread==None:
        stream[url] = CameraStream(id_region,type,url)
    stream[url].set_turn(turn)
    return StreamingResponse(
        video_streaming_generator(stream[url]),
        media_type='multipart/x-mixed-replace; boundary=frame'
    )

@app.get('/api/v1/check-face-real-time/get_rtsp_from_region/test_face_and_plate')
async def get_rtsp_from_region(request: Request):
    id_region = '633eab6969d18929cf048b83'
    ...
    for i in range(len(cameras)):
        ...
        urls = urls+f"?id_region={id_region}&turn={cameras[i]['type']}&rtsp={cameras[i]['face_rtsp_url']}"
        cameras[i]['face_rtsp_url']=urls
        break
    return cameras
